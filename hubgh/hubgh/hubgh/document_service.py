...
def _persist_generated_private_file(file_name, content, attached_to_doctype, attached_to_name):
	"""Persist server-generated bytes as a private File without going through
	``save_file``.

	``save_file`` validates the payload against Frappe's ``max_file_size`` (10 MB
	by default), which is meant for user uploads — not for bundles we build
	server-side from documents that already passed that check individually.
	Writing the bytes to ``private/files`` and creating the File doc from a
	``file_url`` skips that limit, and the MD5 dedup keeps re-downloads from
	piling up orphan copies.
	"""
